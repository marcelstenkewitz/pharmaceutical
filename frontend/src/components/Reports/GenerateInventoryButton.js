...
const GenerateInventoryButton = ({ clientId, reportId, report, variant = "outline-success", size = "sm", className = "" }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const response = await apiService.client.post(`/generate-inventory/${clientId}/${reportId}`, {}, {
      ...
      const filename = `Inventory-Report-${clientName.replace(/[^a-zA-Z0-9]/g, '-')}-${reportId}-${reportDate}.pdf`;
      ...
      if (error.response?.data) {
        try {
          ...
          setError(errorData.error || errorData.details || 'Failed to generate inventory PDF');
        } catch {
          setError(error.response.statusText || 'Failed to generate inventory PDF');
        }
      } else {
        setError(error.message || 'Failed to generate inventory PDF');
      }
    } finally {
  ...
  const handleButtonClick = () => {
    if (!report || !report.lineItems || report.lineItems.length === 0) {
      setError('Cannot generate inventory report: No items found in this report.');
      setShowModal(true);
      return;
    }
    generatePDF();
  ...
  const buttonTitle = `Generate Inventory Report (${stats.total} items${stats.controlled > 0 ? `, ${stats.controlled} controlled` : ''})`;
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            Cannot Generate Inventory Report
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Alert variant="warning">
            <strong>Error:</strong> {error || "This report has no items. Please add items to this report before generating the inventory PDF."}
          </Alert>
...
export default GenerateInventoryButton;